...
{
    //set the Z-Value make snakepart on top of food when colliding.
    setZValue(1);
}
...
void SnakePart::move()
{
    ...
    //every snakepart move based on its direction
    if (direction == "DOWN")
        {
        this->setY(this->y()+40);
        }
    ...
    if(this->getForward()!= NULL)
        {
        //give this path the forward direction
    ...
    // snakepart setimage on its part and forward and backward direction.
    setImage();
...
void SnakePart::checkCollidingObjects()
{
    ...
    for(int i = 0,n = coll.length(); i < n; i++)
    {
        ...
        if(f)
        {
         ...
         //different fruits get different scores.
